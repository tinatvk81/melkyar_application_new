...
from ui.property_form import MoneyLineEdit
...
class DealFormDialog(QDialog):
    def __init__(self, on_saved):
        ...
        self._agents = [u for u in api_client.list_users()
                        if u.get("role") == "agent" and u.get("is_active")]
        ...
        for a in self._agents:
            self.agent_combo.addItem(a["full_name"], a["id"])
...
class PaymentsDialog(QDialog):
    def __init__(self, deal, on_changed):
        super().__init__()
        self.setLayoutDirection(Qt.RightToLeft)
        self.deal = deal
        ...
        self.table.setHorizontalHeaderLabels(["مبلغ", "تاریخ", "توضیح", "رسید"])
    ...
    def _reload(self):
        ...
        for r, p in enumerate(rows):
            ...
            self.table.setItem(r, 1, QTableWidgetItem(p.get("paid_date") or "—"))
            self.table.setItem(r, 2, QTableWidgetItem(p.get("note") or ""))
            self.table.setItem(r, 3, QTableWidgetItem("دارد" if p.get("has_receipt") else "—"))
        self.total_label.setText(
            f"پورسانت: {_money(self.deal['commission_amount'])} — پرداخت‌شده: {_money(total)} — "
            f"مانده: {_money(max(self.deal['commission_amount'] - total, 0))} تومان"
        )
...
class DealsTab(QWidget):
    ...
    def __init__(self):
        ...
        actions = QHBoxLayout()
        for b in (finalize_btn, cancel_btn, payments_btn, pdf_btn):
            actions.addWidget(b)
        actions.addStretch()
    ...
    def load_all(self):
        ...
        for u in users:
            if u.get("role") == "agent":
                self.agent_filter.addItem(u["full_name"], u["id"])
    ...
    def _payments(self):
        ...
        if not d:
            return
        PaymentsDialog(d, on_changed=self.load_all).exec()
